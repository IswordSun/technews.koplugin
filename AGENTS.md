...
1. **构建内存风险**（`technews/epub.lua:44-68`）：整期内容常驻内存；`make_zip` 用 `table.concat` 把所有条目（含图片二进制）拼成一整块 zip 字符串，峰值约为图片字节数的 2 倍。低内存 Kindle 上有隐患。
2. **CRC32 纯 Lua 逐字节循环**（`technews/epub.lua:11-32`）：对约 12MB 载荷逐字节运算，真机上可能造成构建卡顿；需要基准测试。
3. **取消语义不一致**（`main.lua`）：
...
- **B 体验打磨**（今日时间窗口、抓取进度细分、两源去重、图片瘦身、目录层级化、摘要模式补全）：1/6 完成（今日时间窗口已做），其余待办
