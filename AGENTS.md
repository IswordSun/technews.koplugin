...
- 仓库仅本地：分支 `main`，**无 remote**；最新里程碑 tag `v0.1.1`（快照自包含于 `snapshots/`）；基线提交 `4c962b0`，后续改动见 git log
