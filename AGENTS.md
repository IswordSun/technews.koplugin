...
- 远端：GitHub 私有仓库 `origin`（IswordSun/technews.koplugin），**里程碑 + 手动**推送；最新 tag `v0.1.1`（快照自包含于 `snapshots/`）；基线提交 `4c962b0`，后续改动见 git log
...
├── snapshots/                                      ← 里程碑快照（ZIP+bundle，gitignore）
└── technews.koplugin/                              ← 运行时插件（逐字镜像到模拟器/真机）
    ...
    └── technews/
        ...
        ├── favorites.lua                           ← 收藏（单篇快照 EPUB + 索引）
        ├── zipread.lua                             ← 最小 ZIP 读取器（stored；收藏就地取图）
...
| 我的收藏 ▶ | 子菜单：收藏列表（标题 · 收藏日期），点击打开单篇快照；空时显示「暂无收藏」 |
| 管理收藏 ▶ | 子菜单：逐条确认删除（仅在有收藏时出现） |
...
- 阅读器内（打开资讯时）：阅读菜单「收藏 / 取消收藏当前文章」（自动识别当前章节；收藏 = 单篇快照 EPUB 存 `favorites/`，不受缓存清理影响）
...
- `clear_date(date)`：删当天全部 `.epub`；`clear_all()`：删除全部 `.epub`（均连带 `.sdr` 与 `.items.lua` sidecar；`favorites/` 与收藏索引不受影响）
...
4. 重启模拟器实测，用 F8 截图留证
5. 里程碑完成后：更新 §6 与 `technews.koplugin/TODO.md`；打快照 `bash scripts/make_snapshot.sh "<描述>"`（可安装 ZIP + 完整历史 bundle，自包含于 `snapshots/`），视情况升 `main.lua` 版本号并 `git tag`
...
- 与用户沟通用简体中文
- 远端为私有仓库 `origin`（IswordSun/technews.koplugin）；按「里程碑 + 手动」推送（日常提交不推）
...
| GitHub 远端 | `IswordSun/technews.koplugin`（私有） |
